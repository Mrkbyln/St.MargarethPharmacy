...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = false, onClose }) => {
  const { logout, user, lowStockItems, expiredItems } = usePharmacy();
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <div className={`
        fixed top-0 left-0 h-screen bg-[#FCD34D] z-50 w-72 shadow-2xl flex flex-col
      ...
      `}>
        ...
        <div className="pt-8 pb-6 px-6 flex flex-col items-center text-center">
          <div className="relative mb-3">
             <div className="w-20 h-20 rounded-full border-4 border-white/30 overflow-hidden bg-slate-100 shadow-lg">
                <img 
                  src="https://ui-avatars.com/api/?name=St+Margaret&background=f59e0b&color=fff&size=128" 
                  alt="St. Margaret" 
                  className="w-full h-full object-cover"
                />
             </div>
             {/* Decorative pill icon if needed */}
             <div className="absolute -bottom-1 -right-1 bg-white p-1.5 rounded-full shadow-sm">
                <Activity size={16} className="text-amber-500" />
             </div>
          </div>
          
          <h1 className="text-slate-900 font-extrabold text-xl leading-tight">St. Margareth<br/>Pharmacy</h1>
